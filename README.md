...
4. ビルド成功とアセット添付を確認し、下書きを publish する（セルフアップデートは
   publish 済みの最新リリースのみ参照するため、publish しないと更新配信されない）。

詳細な手順・注意点（bump 対象の正確な行、`updater` 署名鍵、CI 不調時のローカル
ビルド）は [CLAUDE.md の「リリース手順」](./CLAUDE.md) を参照。
...
| `src-tauri/src/jira.rs` | Jira ウィンドウ生成・URL/テナント解決・システムメニュー |
| `src-tauri/src/inject.rs` ＋ `inject/*.js` | Jira へ注入する JS。基盤 `machinery.js`（`window.JIRAPP` プラットフォーム）＋各機能（`column_color.js` 等）を `DOC_START_SCRIPTS` で登録 |
